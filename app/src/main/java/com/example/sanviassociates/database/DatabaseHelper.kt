...
class DatabaseHelper(context: Context) : SQLiteOpenHelper(context, DATABASE_NAME, null, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase) {
        val createCustomerTableQuery = """
            CREATE TABLE $CUSTOMER_TABLE (
                ...
                $CUSTOMER_COLUMN_BIRTH_PLACE TEXT,
                $CUSTOMER_COLUMN_BIRTH_DATE TEXT,
                $CUSTOMER_COLUMN_NOMINEE_NAME TEXT,
                $CUSTOMER_COLUMN_NOMINEE_DOB TEXT,
                $CUSTOMER_COLUMN_MOBILE_NUMBER TEXT,
                $CUSTOMER_COLUMN_EMAIL_ID TEXT,
                $CUSTOMER_COLUMN_PAN_CARD TEXT,
                $CUSTOMER_COLUMN_ADHAR_CARD TEXT,
    ...
    override fun onUpgrade(db: SQLiteDatabase, oldVersion: Int, newVersion: Int) {
        if (oldVersion < 3) {
            db.execSQL("ALTER TABLE $CUSTOMER_TABLE ADD COLUMN $CUSTOMER_COLUMN_NOMINEE_NAME TEXT")
            db.execSQL("ALTER TABLE $CUSTOMER_TABLE ADD COLUMN $CUSTOMER_COLUMN_NOMINEE_DOB TEXT")
        }
    }
    ...
    fun getLastCustomerEntryId(): Int {
        val db = readableDatabase
        val cursor = db.rawQuery("SELECT MAX(entry_id) FROM $CUSTOMER_TABLE", null)
        var lastId = 0
        if (cursor.moveToFirst()) {
            lastId = cursor.getInt(0)
        }
